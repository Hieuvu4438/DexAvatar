#!/bin/bash
# Method 4: Multi-Model Ensemble Initialization (with NLF support).
# Mirror of scripts/M3_ensemble_init.sh with one extra optional estimator dir
# (nlf/smplx) added when present. The original script is NOT modified.
#
...
if [ -d "${OUTPUT_PATH}/nlf/smplx" ]; then
    ESTIMATOR_DIRS="$ESTIMATOR_DIRS ${OUTPUT_PATH}/nlf/smplx"
fi
