...
set -e
SIGN="${1:?usage: exp1_paper_nlf_fit.sh <SIGN>}"
DEX=/home/haipd/DexAvatar
DATA="$DEX/outputs/method_nlf_wilor/${SIGN}"
OUT="$DEX/outputs/exp1_paper_nlf/${SIGN}/smplifyx"
...
echo "[exp1] SIGN=$SIGN config=paper.yaml init=nlf/smplx"
python smplifyx/main.py \
    --config cfg_files/fit_smplx_vposer_x_paper.yaml \
    --data_folder "$DATA" \
    --output_folder "$OUT" \
    --img_folder "$FRAMES" \
