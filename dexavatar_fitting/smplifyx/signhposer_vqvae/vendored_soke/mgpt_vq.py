...
Original: mGPT/archs/mgpt_vq.py
Modifications vs. original:
  - Imports of `.tools.resnet` and `.tools.quantize_cnn` replaced with absolute
    imports of the vendored siblings in `vendored_soke/`.
  - Otherwise unchanged.
"""
...
class VQVae(nn.Module):
    ...
    def encode_continuous(self, features: Tensor):
        """Return pre-quantization continuous embedding (differentiable).

        Output shape: (N, T', code_dim) where T' = T // (stride_t ** down_t).
        """
        x_in = self.preprocess(features)
        x_encoder = self.encoder(x_in)  # (N, output_emb_width, T')
        return x_encoder

    def decode_continuous(self, z: Tensor):
        """Decode a continuous embedding (skipping quantization).

        Input shape: (N, code_dim, T').
        Output shape: (N, T, nfeats).
        """
