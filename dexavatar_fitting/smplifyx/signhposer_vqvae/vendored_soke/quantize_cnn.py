...
Original: mGPT/archs/tools/quantize_cnn.py
Minimal vendoring so the SOKE VQVAE has zero SOKE runtime dependencies.
"""
import numpy as np
import torch
...
class QuantizeEMAReset(nn.Module):
    ...
    def reset_codebook(self):
        ...
        # Register on CPU; Module.to(device) moves it to the correct device.
    ...
    def quantize(self, x):
        k_w = self.codebook.t()
        distance = torch.sum(x ** 2, dim=-1, keepdim=True) - 2 * torch.matmul(x, k_w) + \
...
class QuantizeEMA(nn.Module):
    """Same as EMAReset but never resets unused codebook entries."""
    def __init__(self, nb_code, code_dim, mu):
        super().__init__()
        self.nb_code = nb_code
        self.code_dim = code_dim
...
class Quantizer(nn.Module):
    """Original VQ-VAE quantizer (no EMA, no reset)."""
