...
PHASE2R_OUTPUT="${PHASE2R_REPOSITORY}/outputs/phase2r/mesh_aligned_proxy_v2_seed42"
PHASE2R_LOG_ROOT="${PHASE2R_REPOSITORY}/logs/phase2r/mesh_aligned_proxy_v2_seed42"
PHASE2R_CONFIG="${PHASE2R_REPOSITORY}/phase2_refiner/configs/phase2r_mesh_aligned_proxy_v2.yaml"
...
phase2r_fail() {
  ...
  if ((PHASE2R_EXIT_CODE != 0)); then
    ...
    echo "[phase2r-mesh-v2] failed exit=${PHASE2R_EXIT_CODE} $(date --iso-8601=seconds)"
  fi
}
trap phase2r_fail EXIT

cd "${PHASE2R_REPOSITORY}"
if [[ -d "${PHASE2R_OUTPUT}" ]] && [[ -n "$(find "${PHASE2R_OUTPUT}" -mindepth 1 -maxdepth 1 -print -quit)" ]]; then
  echo "Refusing non-empty append-only output: ${PHASE2R_OUTPUT}" >&2
  exit 2
fi

echo "PREFLIGHT" > "${PHASE2R_LOG_ROOT}/status"
python - <<'PY'
...
config = load_config("phase2_refiner/configs/phase2r_mesh_aligned_proxy_v2.yaml")
validate_config(config, require_data=True, require_validation=True)
assert config["data"]["formal_evidence"] is False
...
print("[phase2r-mesh-v2] proxy-only cache preflight passed", flush=True)
PY
...
while true; do
  ...
  echo "[phase2r-mesh-v2] waiting for GPU: free=${PHASE2R_FREE_MIB:-unknown} MiB required=${PHASE2R_MINIMUM_FREE_MIB} MiB"
  sleep 60
done

echo "TRAINING" > "${PHASE2R_LOG_ROOT}/status"
python -u -m phase2_refiner.train --config "${PHASE2R_CONFIG}" --device cuda
...
echo "[phase2r-mesh-v2] complete $(date --iso-8601=seconds)"
trap - EXIT
