#!/usr/bin/env python3
"""NLF body + HaMeR hands entry point.

Mirror of methods/run_dexavatar.py with the bash invocation pointing at
Full_running_command_nlf_hamer.sh.

Usage:
    python methods/run_dexavatar_nlf_hamer.py \
        --input_img_folder data/frames \
        --output_path outputs/nlf_hamer \
...
for sub_folder in sub_folder_list:
    ...
    cmd = (
        ...
        f"bash -c 'unset LD_LIBRARY_PATH && bash {SCRIPT_DIR}/Full_running_command_nlf_hamer.sh'"
    )
    print("Running:", cmd)
    os.system(cmd)
