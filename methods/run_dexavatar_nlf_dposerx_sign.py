"""
Runner for the NLF + WiLoR + VQVAE-hand + SIGN-TRAINED DPoser-X body pipeline.
Reuses existing pre-extracted Sapiens/WiLoR stages. (Variant of
run_dexavatar_nlf_vqvae_dposerx.py; old runner unchanged.)

Usage:
    python methods/run_dexavatar_nlf_dposerx_sign.py \
        --input_img_folder data/frames \
        --output_path outputs/method_nlf_dposerx_sign \
...
parser.add_argument('--output_path', type=str,
                    default='outputs/method_nlf_dposerx_sign')
...
for sub_folder in sub_folder_list:
    ...
    cmd = (
        ...
        f"bash -c 'unset LD_LIBRARY_PATH && bash {SCRIPT_DIR}/Full_running_command_nlf_dposerx_sign.sh'"
    )
    print("Running:", cmd)
    os.system(cmd)
