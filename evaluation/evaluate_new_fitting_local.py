...
def main(
    ...
    for idx, (soma_key, method_key) in enumerate(tqdm(soma_to_method.items(), desc='Signs')):
        ...
        # Skip signs with no mocap meshes
        if len(mocap_objs[idx]) == 0:
            logger.warning(f'Skipping {soma_key}: no mocap meshes found')
            continue

        # Use min frame count to handle mismatches gracefully
        n_frames = min(len(gt_objs[idx]), len(mocap_objs[idx]))
        if len(gt_objs[idx]) != len(mocap_objs[idx]):
            logger.warning(f'{soma_key}: frame count mismatch GT={len(gt_objs[idx])} vs BL={len(mocap_objs[idx])}, using {n_frames}')
        ...
        for inter_idx in range(n_frames):
