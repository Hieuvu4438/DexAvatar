#!/usr/bin/env python3
"""
PA-MPVPE evaluation for SignEFT-X SMPL-X meshes.
Standardized Procrustes-Aligned Mean Per-Vertex Position Error (PA-MPVPE) and MPVPE benchmark.

Protocol implementation copied from ``signal4d/evaluate_pampvpe.py``
(source SHA-256: cda6aa1856022407930669ec80e11854f78a90d3f3af51948305e20fb4202f0e).
...
class SMPLXAssets:
    def __init__(self, data_base_dir: str, face_indices: str):
        ...
        self.face_idx = np.load(face_indices).astype(np.int64)
...
def evaluate_dataset(
    ...
    if set(frame_segment) != set(sign_list):
        raise ValueError("sign/segment metadata mismatch")
...
def print_summary_table(summary: Dict[str, Any]) -> None:
    ...
    print("\n" + "=" * 82)
    print("               PA-MPVPE EVALUATION REPORT (SignEFT-X)")
    print("=" * 82)
...
def parse_args():
    ...
    parser.add_argument('--sign_file', type=str, required=True,
                        help='Path to signs.txt')
    parser.add_argument('--sign_seg', type=str, required=True,
                        help='Path to segment.json')
    parser.add_argument('--data_base_dir', type=str, required=True,
                        help='Path to SMPL-X / MANO data directory')
    parser.add_argument('--face_indices', type=str, required=True,
                        help='Path to SMPL-X__FLAME_vertex_ids.npy')
...
def main():
    ...
    assets = SMPLXAssets(args.data_base_dir, args.face_indices)
