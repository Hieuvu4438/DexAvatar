...
def external_frame_paths(root: Path, record: FrameRecord) -> ExternalFrame:
    base = root / record.sign / "smplifyx"
    stem = f"low_{record.source_frame_id}"
    return ExternalFrame(record, base / "results" / f"{stem}.pkl", base / "meshes" / f"{stem}.obj")


def load_external_parameters(path: Path) -> dict[str, np.ndarray]:
    # These are trusted, local artifacts produced by the reused External V1 pipeline.
...
def _all_external_frames(manifest_root: Path, external_root: Path) -> list[ExternalFrame]:
    frames = []
    for manifest_path in sorted(manifest_root.glob("*.jsonl")):
        frames.extend(external_frame_paths(external_root, record) for record in read_jsonl(manifest_path))
    missing = [str(path) for frame in frames for path in (frame.result_path, frame.mesh_path) if not path.is_file()]
    if missing:
        raise FileNotFoundError(f"Missing {len(missing)} External V1 artifacts; first: {missing[0]}")
    return frames
...
def calibrate_external_identity(
    external_root: Path,
    manifest_root: Path,
    output_npz: Path,
    ...
    observations = []
    for frame in _all_external_frames(manifest_root, external_root):
        parameters = load_external_parameters(frame.result_path)
    ...
    if refine_steps:
        ...
        target = tensor(np.stack([load_obj_vertices(frame.mesh_path) for frame in frames]))
        target = target * tensor(np.asarray([1.0, -1.0, -1.0], dtype=np.float32))
        model = smplx.create(
        ...
        for _ in range(refine_steps):
            ...
            whole_loss = torch.square(vertices - target).mean()
    ...
    report = {
        "schema_version": "signpccx.external-identity.v1",
        "scope": "signer",
        "source": str(external_root.resolve()),
...
def _stack_parameters(frames: list[ExternalFrame]) -> dict[str, np.ndarray]:
    loaded = [load_external_parameters(frame.result_path) for frame in frames]
    return {key: np.stack([item[key] for item in loaded]) for key in PARAMETER_SHAPES}


def canonical_refit_external(
    external_root: Path,
    manifest_root: Path,
    identity_npz: Path,
    model_root: Path,
    ...
    *,
    ...
    for manifest_path in manifest_paths:
        ...
        if destination.exists() or sign_report_path.exists():
            ...
            continue
        frames = [external_frame_paths(external_root, record) for record in records]
        parameters = _stack_parameters(frames)
        sign_vertices = []
        fitted_body_pose = []
        ...
        for chunk_start in range(0, len(frames), chunk_size):
            ...
            tensor = lambda value: torch.as_tensor(value, dtype=torch.float32, device=device)
            target_np = np.stack([load_obj_vertices(frame.mesh_path) for frame in chunk]) * boundary_x180
            ...
            with torch.no_grad():
                ...
                initial_hand_mm = float(
                    torch.linalg.vector_norm(initial_vertices[:, hand_ids] - target[:, hand_ids], dim=-1).mean() * 1000
                )
            best_loss = float("inf")
            best_deltas = None
            stale = 0
            ...
            for step in range(steps):
                ...
                target_left = target[:, left_ids]
                target_right = target[:, right_ids]
        ...
        np.savez_compressed(
            ...
            left_hand_teacher_residual_mm=np.concatenate(left_residual_per_frame),
            right_hand_teacher_residual_mm=np.concatenate(right_residual_per_frame),
        )
        sign_report = {
    ...
    report = {
        "schema_version": "signpccx.canonical-refit.v2",
        "source": str(external_root.resolve()),
    ...
    if signs is None:
        ...
        atomic_write_json(output_root / "partial_run_manifest.json", report)
    return report
